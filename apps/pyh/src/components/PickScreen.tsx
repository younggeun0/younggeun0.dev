...
function PickScreen() {
  ...
  const addHold = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement, MouseEvent>, isTop: boolean = false): void => {
      const canvas = refCanvas.current
      if (canvas) {
  ...
  const drawPoints = useCallback((): void => {
    ...
    if (canvas) {
      ...
      if (ctx) {
        ...
        points.map((point: { x: number; y: number; isTop: any }, index: number) => {
          ctx.beginPath()
    ...
    <div className="p-4">
      {isLoading && (
        <div className="flex flex-col justify-center items-center h-96 space-y-4">
          ...
          <p className="text-gray-300 text-center mt-4">이미지를 선택하지 않고 닫은 경우 새로고침 부탁드립니다.</p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center space-x-4">
            <Button variant="ghost" size="icon" onClick={undo} aria-label="undo">
              <img className="w-12" src="img/grin.png" alt="undo" />
            </Button>
            <Button variant="ghost" size="icon" onClick={reset} aria-label="reset">
              <img className="w-12" src="img/sparkle.png" alt="reset" />
            </Button>
            <Button variant="ghost" size="icon" onClick={recapture} aria-label="recapture">
              <img className="w-12" src="img/camera.png" alt="recapture" />
            </Button>
            <Button variant="ghost" size="icon" onClick={save} aria-label="save">
              <img className="w-12" src="img/disk.png" alt="save" />
            </Button>
          </div>
        </div>
      )}

      <img src={defaultImage} className="hidden" onLoad={updateCanvas} ref={refHiddenImg} />
      <input
        type="file"
        accept="image/*"
        capture="environment"
